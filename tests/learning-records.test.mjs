...
test('empty records stay empty; submitted practice uses frozen reference after closure',()=>{
 const s=store();assert.deepEqual(learningRecords(s,user),[]);const [q]=availablePractice(seedQuestions(s));submitPractice(s,user,q,'known');const records=learningRecords(s,user);assert.equal(records.length,1);assert.equal(wrongQuestions(records).length,1);assert.equal(records[0].items[0].answer,'已送检组织');assert.deepEqual(learningRecords(s,{id:'other',roles:['student']}),[]);
});
test('unreleased exams expose no score or reference; correction updates wrong list without duplicates',()=>{
 ...
 let rows=learningRecords(s,user);assert.equal(rows[0].items.length,0);assert.equal(rows[0].total,null);assert.equal(wrongQuestions(rows).length,0);
 ...
 const releases=[{examId:'e',version:1,results:[result]}];s.setItem(gradingKey,JSON.stringify({reviews:[],releases}));rows=learningRecords(s,user);assert.equal(wrongQuestions(rows).length,1);assert.equal(rows[0].items[0].image,'/old.png');
