...
export function learningRecords(storage,user){
 ...
 const practice=readPractice(storage,user).filter(r=>r.studentId===user.id).map(r=>{
  const q=r.question;return {id:r.id,source:'practice',title:q.prompt,time:r.submittedAt,state:'已提交',version:q.revision,total:null,reason:'',items:[{id:q.id,type:q.type,prompt:q.prompt,answer:q.type==='single'?(q.options.find(o=>o.id===r.answer)?.text||'未作答'):r.answer,reference:q.type==='single'?(q.options.find(o=>o.id===q.correctId)?.text||''):q.reference,explanation:q.explanation||'',comment:'',image:q.image,history:q.caseSnapshot.history,organ:q.caseSnapshot.organ||'未分类',wrong:q.type==='single'&&r.correct===false,score:null,maxScore:null}]};
 });
 const exams=readAttempts(storage).filter(a=>a.studentId===user.id).map(a=>{
  const grade=studentGrade(storage,a.id,user);
  return {id:a.id,source:'exam',title:a.exam.title,time:new Date(a.submittedAt||a.startedAt).toISOString(),state:grade?'已发布':a.status==='submitted'?'待发布':'进行中',version:grade?.releaseVersion||a.exam.version,total:grade?.total??null,reason:grade?.reason||'',items:grade?grade.scores.map(s=>{const q=a.exam.questions.find(q=>q.id===s.questionId),c=a.exam.cases.find(c=>c.id===q?.caseId);return {id:s.questionId,type:s.type,prompt:s.prompt,answer:s.type==='single'?(s.options.split('\n')[Number(s.answer)-1]||'未作答'):s.answer,reference:s.type==='single'?(s.options.split('\n')[Number(s.reference)-1]||''):s.reference,explanation:s.explanation,comment:s.comment,image:c?.images?.[0]||'',history:c?.history||'',organ:c?.organ||'未分类',wrong:s.type==='single'&&s.score<s.maxScore,score:s.score,maxScore:s.maxScore};}):[]};
 });return [...practice,...exams].sort((a,b)=>Date.parse(b.time)-Date.parse(a.time));
}
/** @returns {any[]} */
export function wrongQuestions(records){return records.flatMap(r=>r.items.filter(q=>q.wrong).map(q=>({...q,key:r.id+':'+q.id,recordId:r.id,source:r.source,title:r.title,time:r.time,version:r.version,reason:r.reason})));}
