...
export function gradingProgress(attempts,data,examId){
 const submitted=attempts.filter(a=>a.examId===examId&&a.status==='submitted');
 const reviewed=submitted.filter(a=>data.reviews.some(r=>r.attemptId===a.id));
 const release=data.releases.filter(r=>r.examId===examId).at(-1);
 return {submitted:submitted.length,reviewed:reviewed.length,pending:submitted.length-reviewed.length,published:submitted.filter(a=>release?.results.some(r=>r.attemptId===a.id)).length};
}
