...
export function startAttempt(storage,examId,user,now=Date.now()){
 const exam=paper(storage,examId,user),rows=readAttempts(storage),existing=rows.find(a=>a.examId===examId&&a.studentId===user.id);if(existing)return existing;
 ...
 storage.setItem(attemptsKey,JSON.stringify([...rows,attempt]));return attempt;
}
