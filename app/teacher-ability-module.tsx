...
export function TeacherAbilityModule({ onBack, initialView="overview" }: { onBack: () => void; initialView?:"overview"|"records" }) {
  ...
  if(dimension)return <section className="account-card"><button type="button" className="page-back" onClick={()=>setDimension(null)}><span aria-hidden="true">←</span> 返回能力总览</button><h1>{dimension} · 证据集合</h1><p>示例证据</p>{dimension==="资源建设"?<p>暂无可追溯的资源贡献记录，数据不足。</p>:<><h2>{records[0].title}</h2><p>{records[0].id}</p><button onClick={()=>{setRecordId(records[0].id);setAnalysisTab(dimension==="专业知识"?"diagnosis":dimension==="教学技能"?"qa":"diagnosis");setSourceView("overview");setSourceDimension(dimension);setView("detail");setDimension(null);}}>查看此记录的对应证据</button></>}</section>;
  if(view==="choose")return <section className="account-card"><button type="button" className="page-back" onClick={()=>setView("records")}><span aria-hidden="true">←</span> 返回教学记录</button><h1>选择教学病例</h1><p>仅胃病例提供完整流程示意，其他病例尚无专属教学素材。</p>{records.map((r,i)=><article key={r.id}><h2>{r.title}</h2><button disabled={i!==0} title={i!==0?"尚无专属教学素材":undefined} onClick={()=>{setRecordId(r.id);setView("capture");}}>使用此病例开始阅片</button></article>)}</section>;
  if (view === "capture") {
    return <section className="teacher-capture">
      <header><button type="button" className="page-back" onClick={() => {setRecording(false);returnToSource();}}><span aria-hidden="true">←</span> 返回上级页面</button><div><b>胃中分化腺癌教学阅片</b><small>LYND00600 · 教学分析记录 TA-20260831-01</small></div><span className={recording ? "live" : ""}>{recording ? `● 演示中 ${elapsed}` : "未开始"}</span></header>
  ...
  if (view === "detail") {
    ...
    if(record.id!==records[0].id)return <section className="account-card"><button type="button" className="page-back" onClick={returnToSource}><span aria-hidden="true">←</span> 返回上级页面</button><h1>{record.title}</h1><p>{record.id} · {record.status}</p><p>此记录尚无专属证据，不能展示其他病例的分析结果。</p></section>;
    return <section className="teacher-analysis-detail">
      <p className="library-muted">录音分析示例 · 预设结果</p>
      <header><button type="button" className="page-back" onClick={returnToSource}><span aria-hidden="true">←</span> 返回{sourceDimension?"维度证据":sourceView==="overview"?"能力总览":"教学记录"}</button><div><h2>{record.title}</h2><p>{record.id} · {record.date} · {record.duration}</p></div><span>{record.status}</span></header>
  ...
  return <section className="teacher-ability">
    ...
    {view === "overview" ? <>
      ...
      <div className="teacher-overview-grid"><article className="teacher-radar"><header><div><h3>四维能力画像</h3><p>教学记录中的可回看示例</p></div><span>未评定分数</span></header><div className="dimension-chart" role="img" aria-label="四个维度可回看的证据条数（示例）">{dimensions.map(d=><div className="dimension-bar" key={d.name}><span className="dimension-bar-name">{d.name}</span><span className="dimension-bar-track">{d.state==='数据不足'?<em>现有 {d.evidence} 条，不足以形成结论</em>:<i className={'state-'+d.state} style={{width:Math.round(d.evidence/Math.max(...dimensions.map(x=>x.evidence))*100)+'%'}}/>}</span><span className="dimension-bar-value">{d.state==='数据不足'?'未成结论':d.evidence+' 条'}</span></div>)}</div><p>专业知识、教学技能、AI应用、资源建设四个维度分别回看教学依据。</p><p>当前可回看：胃中分化腺癌教学阅片。其他记录暂缺专属证据。</p><footer>示例记录日期：2026-08-28 · 条形长度＝可回看证据条数（示例），未生成量化评分</footer></article><article className="teacher-summary"><h3>近期结论</h3><div className="teacher-highlight advantage"><b>优势</b><p>能够从整体结构进入高倍证据，并对 AI 初步结论进行核验后再采纳。</p><button type="button" onClick={() => {setSourceView(view==="overview"?"overview":"records");setSourceDimension("");setRecordId(records[0].id);setView("detail");}}>回看诊断证据 →</button></div><div className="teacher-highlight improve"><b>待提升</b><p>鉴别诊断讲解较完整，但引导学生主动提出判断依据的次数较少。</p><button type="button" onClick={() => {setRecordId(records[0].id);setView("detail");}}>回看互动证据 →</button></div></article></div>
