...
export default function LibraryWorkspace({user,onCreateQuestion}:{user:Account;onCreateQuestion?:(id:string)=>void}) {
  ...
  return <section className="library-workspace">
    <header className="library-header">{selected&&<Button className="page-back" onClick={()=>show('')}><span aria-hidden="true">←</span> 返回图书馆</Button>}<div><p className="account-eyebrow">教学资源</p><h1 ref={heading} tabIndex={-1}>{selected?(item?.title||'病例不可用'):'切片图书馆'}</h1><p>{selected?'临床资料与图片':'检索、收藏并查看开放的教学病例'}</p></div><div className="library-header-actions">{!selected&&<nav className="library-tabs" aria-label="资料范围"><Button variant={!onlyFavorites?'primary':'secondary'} aria-pressed={!onlyFavorites} onClick={()=>{setOnlyFavorites(false);setPage(1);}}>全部切片</Button><Button variant={onlyFavorites?'primary':'secondary'} aria-pressed={onlyFavorites} onClick={()=>{setOnlyFavorites(true);setPage(1);}}>我的收藏（{cases.filter(c=>favorites.includes(c.id)).length}）</Button></nav>}{!selected&&teacher&&<Button variant="primary" onClick={()=>setEditing('new')}>新增病例</Button>}</div></header>
    ...
    {selected?(item?<>
      ...
      <CaseViewer key={item.id+':'+item.version} item={item}/>
