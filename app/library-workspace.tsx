...
export default function LibraryWorkspace({user,onCreateQuestion}:{user:Account;onCreateQuestion?:(id:string)=>void}) {
  ...
  return <section className="library-workspace">
    <header className="library-header"><div><p className="account-eyebrow">教学资源</p><h1 ref={heading} tabIndex={-1}>{selected?(item?.title||'病例不可用'):'切片图书馆'}</h1><p>{selected?'临床资料与图片':'检索、收藏并查看开放的教学病例'}</p></div><div>{selected?<Button onClick={()=>show('')}>返回图书馆</Button>:teacher&&<Button variant="primary" onClick={()=>setEditing('new')}>新增病例</Button>}</div></header>
    {error&&<p role="alert" className="account-error">{error}</p>}
    {!ready&&!error&&<p role="status">正在加载资料…</p>}
    {selected?(item?<>
      <div className="library-detail-actions"><span className="library-tag">{item.library} · v{item.version}</span>{item.simulated&&<span className="library-tag">模拟病例 · 共用占位图</span>}<Button onClick={()=>favorite(item.id)}>{favorites.includes(item.id)?'取消收藏':'收藏'}</Button>{teacher&&item.owner===user.id&&<Button onClick={()=>setEditing(item.id)}>编辑病例与参考</Button>}</div>
      <div className="library-detail"><aside className="account-card"><h2>临床资料</h2><dl><dt>教学编号</dt><dd>{item.id}</dd><dt>分类</dt><dd>{item.organ||'未提供'}</dd><dt>难度</dt><dd>{item.difficulty||'未提供'}</dd></dl>{item.clinical&&Object.values(item.clinical).some(Boolean)?Object.entries(item.clinical).map(([name,text])=><section key={name}><h3>{name}</h3><p>{text||'未提供'}</p></section>):<p className="pre-wrap">{item.history||'未提供病史'}</p>}<h3>资料来源</h3><p>{item.source||'未提供'}</p></aside>
      <ImageReader key={item.id+':'+item.version} images={item.images}/></div>
      {teacher&&<section className="account-card library-reference"><h2>教师参考资料</h2><p className="pre-wrap">{item.reference||'尚未填写参考资料'}</p>{onCreateQuestion&&<Button variant="primary" onClick={()=>onCreateQuestion(item.id)}>用于出题</Button>}</section>}
    </>:<div className="account-card"><p>此病例不存在或未向当前账号开放。</p><Button onClick={()=>show('')}>返回列表</Button></div>):<>
      <nav className="library-tabs" aria-label="资料范围"><Button variant={!onlyFavorites?'primary':'secondary'} aria-pressed={!onlyFavorites} onClick={()=>{setOnlyFavorites(false);setPage(1);}}>全部切片</Button><Button variant={onlyFavorites?'primary':'secondary'} aria-pressed={onlyFavorites} onClick={()=>{setOnlyFavorites(true);setPage(1);}}>我的收藏（{cases.filter(c=>favorites.includes(c.id)).length}）</Button></nav>
      <div className="library-toolbar"><label>搜索病例<input placeholder="病例名称、编号或部位" value={query} onChange={e=>{setQuery(e.target.value);setPage(1);}}/></label><SearchSelect label="部位" value={category} options={allCategories} onChange={value=>{setCategory(value);setPage(1);}}/><label>数据库来源<select value={source} onChange={e=>{setSource(e.target.value);setPage(1);}}><option value="">全部来源</option><option>标准库</option><option>院方库</option></select></label><Button onClick={()=>{setQuery('');setCategory('');setSource('');setPage(1);}}>清空筛选</Button></div>
